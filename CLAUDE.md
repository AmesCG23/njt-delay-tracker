...
5. **Data commentary (conditional)** — one plain line of bigger-picture context, only on days a detector actually fires. `_analysis_candidates()` in `composer.py` is the full palette: a cumulative milestone crossed, Penn/Portal recurring system-wide frequency (k of the last 10 days), a record-chasing streak of above-average days, ranking among all days (Nth-worst/lightest), a relatable equivalence ("as much lost time as N average days combined") on outsized days, a calmest-stretch streak, a turning point (a streak just broke), and 7-day momentum vs. the 30-day average. `_select_analysis()` picks the single highest-salience candidate and skips any angle that already appeared in the most recent post. Needs 30 days of history to run at all (same gate as beat 3); ranking/record-chase claims additionally wait for `ANALYSIS_MIN_HISTORY` (60 days) so "3rd-worst of 40" can't be claimed. **Beat 5 is the only discretionary element in the framework** — beats 1–4 are always present, and the model draws its variety solely from whichever beat-5 detector fires.

**Line/hub and morning-vs-evening commentary were tried and retired (not just gated).** Two earlier iterations offered the model a "hardest-hit line" extra (`_worst_driver`, later narrowed to fire only on a genuine systemwide event — Penn Station, a Hoboken diversion, a full line suspension) and a `heavier_rush` morning/evening split, both surfaced as always-available "secondary detail" outside the beat-5 palette. Both still settled into one answer almost every day — Northeast Corridor kept winning the raw-hours comparison purely because it's the busiest line, and the evening rush was almost always the heavier one — so instead of adding variety to the posts, they added repetition. Rather than narrow the gate further, both were removed outright: `_worst_driver`, `_heavier_rush`, and the "Secondary detail" block in `build_task_prompt` are gone, and `compute_stats()`/`compose_post()` no longer take `morning_totals`/`evening_totals` at all. If either angle is wanted back, it belongs as a proper beat-5 candidate in `_analysis_candidates()` (salience-scored, anti-repetition-checked) rather than an unconditional freebie.
